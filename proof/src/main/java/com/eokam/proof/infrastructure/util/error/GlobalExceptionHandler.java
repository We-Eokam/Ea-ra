...
public class GlobalExceptionHandler {
	...
	protected ResponseEntity<ErrorResponse> handleMethodArgumentTypeMismatchException(
		...
		ErrorResponse errorResponse =
			ErrorResponse.create(exception, HttpStatus.BAD_REQUEST, "Argument 타입이 일치하지 않습니다.");
		return ResponseEntity.badRequest().body(errorResponse);
	}
	...
	protected ResponseEntity<ErrorResponse> handleMissingServletRequestPartException(
		HttpMessageNotReadableException exception) {
		log.error("handleMissingServletRequestPartException", exception);
		ErrorResponse errorResponse =
			ErrorResponse.create(exception, HttpStatus.BAD_REQUEST, "multipart가 입력되지 않았거나 손실되었습니다.");
		return ResponseEntity.badRequest().body(errorResponse);
	}
